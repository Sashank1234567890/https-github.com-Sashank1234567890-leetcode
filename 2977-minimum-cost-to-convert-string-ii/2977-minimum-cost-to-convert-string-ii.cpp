class Solution
{
        ...
        while (!pq.empty())
        {
            ...
            for (auto &edge: adj[node])
            {
                ...
                if (!result.count(adjNode) || currCost + edgeCost < result[adjNode])
                {
                    ...
                    pq.push({ currCost + edgeCost,
                        adjNode });
                }
            }
        }

        ll finalCost = result.count(end) ? result[end] : invalid_val;
    ...
    long long minimumCost(string source, string target, vector<string> &original, vector<string> &changed, vector< int > &cost)
    {
        ...
        {
            adj[original[i]].push_back({ changed[i],
                cost[i] });
        }
        ...
        return result == invalid_val ? -1 : result;
    }
};